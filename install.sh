...
install_ubuntu(){
  ...
  if [[ -d "$PREFIX/var/lib/proot-distro/installed-rootfs/ubuntu" ]]; then
    echo "${G}Existing Ubuntu installation found, Reinstalling it...${W}"
    proot-distro reset ubuntu
  else
    echo "${G}Installing Ubuntu, Depends on your network and device...${W}"
    echo
    pkg update
...
install_desktop(){
  echo "${G}Installing XFCE Desktop View...${W}"
...
final_banner(){
  banner
  echo
  echo "${G}Ubuntu Is Ready To Start!!"
  echo
  echo
  echo "COMMANDS"
  echo
  echo
  echo "ubuntu  -  It will Start Your Ubuntu"
  echo
  echo "vncstart  -  To start vncserver, Execute inside proot-distro login ubuntu"
  echo
  echo "vncstop  -  To stop vncserver, Execute inside like vncstart${W}"
  echo
  echo "Enjoy Your Ubuntu!!"
}
